...
RSpec.describe :when do
  it "runs body when condition passes" do
    msg = rand_str
    dotty_run_script "((:when \"true\" (:debug #{msg.inspect}) ))" do |dotty, _, _, serr|
  ...
  it "doesn't runs body when condition fails" do
    msg = rand_str
    dotty_run_script "((:when \"false\" (:debug #{msg.inspect}) ))" do |dotty, _, _, serr|
  ...
  it "makes conditions non-interactive by default" do
    msg = rand_str
    dotty_run_script "((:when \"echo #{msg}\" (:debug \"\") ))" do |dotty, _, sout|
  ...
  it "lets conditions be interactive if desired" do
    msg = rand_str
    dotty_run_script "((:when {:cmd \"echo #{msg}\" :stdout true} (:debug \"\") ))" do |dotty, _, sout|
  ...
  it "supports negation" do
    msg = rand_str
    dotty_run_script "((:when (:not \"false\") (:debug #{msg.inspect}) ))" do |dotty, _, _, serr|
      expect(serr.read().uncolorize).to match(/DBG #{msg}/)
    end
    dotty_run_script "((:when (:not \"true\") (:debug #{msg.inspect}) ))" do |dotty, _, _, serr|
      expect(serr.read().uncolorize).not_to match(/DBG #{msg}/)
    end
  end
  ...
  it "supports conjunction" do
    msg = rand_str
    dotty_run_script "((:when (:and \"true\" \"true\") (:debug #{msg.inspect}) ))" do |dotty, _, _, serr|
      expect(serr.read().uncolorize).to match(/DBG #{msg}/)
    end
    dotty_run_script "((:when (:and \"true\" \"false\") (:debug #{msg.inspect}) ))" do |dotty, _, _, serr|
      expect(serr.read().uncolorize).not_to match(/DBG #{msg}/)
    end
  end
  ...
  it "supports disjunction" do
    msg = rand_str
    dotty_run_script "((:when (:or \"true\" \"true\") (:debug #{msg.inspect}) ))" do |dotty, _, _, serr|
      expect(serr.read().uncolorize).to match(/DBG #{msg}/)
    end
    dotty_run_script "((:when (:or \"false\" \"false\") (:debug #{msg.inspect}) ))" do |dotty, _, _, serr|
      expect(serr.read().uncolorize).not_to match(/DBG #{msg}/)
    end
  end
